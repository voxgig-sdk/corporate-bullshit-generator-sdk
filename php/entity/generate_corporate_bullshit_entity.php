...
// CorporateBullshitGenerator SDK GenerateCorporateBullshit entity
...
class GenerateCorporateBullshitEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "generate_corporate_bullshit";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GenerateCorporateBullshitEntity($this->_client, $opts);
    }

    /**
     * @param GenerateCorporateBullshit|array $args GenerateCorporateBullshit data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CorporateBullshitGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GenerateCorporateBullshit|array The current GenerateCorporateBullshit data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GenerateCorporateBullshit fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CorporateBullshitGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GenerateCorporateBullshit fields).
     */
    ...
    /**
     * Load a single GenerateCorporateBullshit.
     *
     * @param GenerateCorporateBullshitLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GenerateCorporateBullshitLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GenerateCorporateBullshit|array The loaded GenerateCorporateBullshit as an assoc-array at the
     *   SDK boundary; throws CorporateBullshitGeneratorError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CorporateBullshitGeneratorHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
