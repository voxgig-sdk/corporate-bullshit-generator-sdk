...
class GenerateCorporateBullshitEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CorporateBullshitGeneratorSDK::test(null, null);
        $ent = $testsdk->GenerateCorporateBullshit(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = generate_corporate_bullshit_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "generate_corporate_bullshit." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $generate_corporate_bullshit_ref01_data = null;
        if (count($generate_corporate_bullshit_ref01_data_raw) > 0) {
...
function generate_corporate_bullshit_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/generate_corporate_bullshit/GenerateCorporateBullshitTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["generate_corporate_bullshit01", "generate_corporate_bullshit02", "generate_corporate_bullshit03"] as $k) {
