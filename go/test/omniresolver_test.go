...
import (
	...
	omni "github.com/voxgig-sdk/corporate-bullshit-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CorporateBullshitGeneratorError); is {
				out := map[string]any{
					"name":    "CorporateBullshitGeneratorError",
...
func omniCtx(arg any, client *sdk.CorporateBullshitGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CorporateBullshitGeneratorSDK); is {
			client = live
