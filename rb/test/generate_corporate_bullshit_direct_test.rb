...
class GenerateCorporateBullshitDirectTest < Minitest::Test
  def test_direct_load_generate_corporate_bullshit
    setup = generate_corporate_bullshit_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-generate_corporate_bullshit", setup[:live] ? "live" : "unit")
    if _should_skip
...
def generate_corporate_bullshit_direct_setup(mockres)
