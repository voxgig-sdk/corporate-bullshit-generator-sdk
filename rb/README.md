...
The Ruby SDK for the CorporateBullshitGenerator API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.GenerateCorporateBullshit(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.CorporateBullshitGenerator(nil).load(
  { "id" => "test01" }, nil
)
