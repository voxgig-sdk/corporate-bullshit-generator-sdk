# corporate-bullshit-generator-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CorporateBullshitGenerator SDK as
two agent tools — `corporate-bullshit-generator_list` and `corporate-bullshit-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/corporate-bullshit-generator-mcp)
make build

# 2. Provide credentials via the environment
export CORPORATE_BULLSHIT_GENERATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user corporate-bullshit-generator \
  -- /absolute/path/to/corporate-bullshit-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./corporate-bullshit-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// corporate-bullshit-generator_list: first page of records
{ "entity": "generate_corporate_bullshit" }
{ "entity": "generate_corporate_bullshit", "query": { } }

// corporate-bullshit-generator_load: one record by id
{ "entity": "generate_corporate_bullshit", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/corporate-bullshit-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user corporate-bullshit-generator \
     -- "$PWD"/dist/*/corporate-bullshit-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `corporate-bullshit-generator_list` and `corporate-bullshit-generator_load` tools now appear
   in new sessions. Ask the agent to *"list generate_corporate_bullshit using corporate-bullshit-generator"*
   and it calls `corporate-bullshit-generator_list` with `{"entity":"generate_corporate_bullshit"}`.
...
```sh
export CORPORATE_BULLSHIT_GENERATOR_APIKEY=sk_live_xxx            # API key
export CORPORATE_BULLSHIT_GENERATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `corporate-bullshit-generator_list` tool
...
### Call the `corporate-bullshit-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `corporate-bullshit-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `corporate-bullshit-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CORPORATE_BULLSHIT_GENERATOR_APIKEY` | API key sent with every request. |
| `CORPORATE_BULLSHIT_GENERATOR_BASE` | Optional override of the API base URL. |
...
```sh
./corporate-bullshit-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"corporate-bullshit-generator_load","arguments":{"entity":"generate_corporate_bullshit","query":{"id":1}}}}'
```
