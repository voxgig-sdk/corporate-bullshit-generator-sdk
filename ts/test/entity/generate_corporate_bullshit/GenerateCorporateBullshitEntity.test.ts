...
describe('GenerateCorporateBullshitEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'generate_corporate_bullshit.' + op, live)) return
    }
    ...
    const generate_corporate_bullshit_ref01_data_dt0 = await generate_corporate_bullshit_ref01_ent.load(generate_corporate_bullshit_ref01_match_dt0)
