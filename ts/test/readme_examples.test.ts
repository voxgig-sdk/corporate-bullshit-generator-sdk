...
//     (\`new CorporateBullshitGeneratorSDK(...)\` / \`CorporateBullshitGeneratorSDK.test(...)\` become
//     \`CorporateBullshitGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CorporateBullshitGeneratorSDK'
...
const TEST_SEED = {"entity":{"generate_corporate_bullshit":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CorporateBullshitGeneratorSDK(...)\` and any
// \`CorporateBullshitGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CorporateBullshitGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CorporateBullshitGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
