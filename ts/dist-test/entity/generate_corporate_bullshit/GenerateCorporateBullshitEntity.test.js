...
(0, node_test_1.describe)('GenerateCorporateBullshitEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when CORPORATE_BULLSHIT_GENERATOR_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('CORPORATE_BULLSHIT_GENERATOR_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.CorporateBullshitGeneratorSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        (0, node_assert_1.default)(null != generate_corporate_bullshit_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/generate_corporate_bullshit/GenerateCorporateBullshitTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.CorporateBullshitGeneratorSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['generate_corporate_bullshit01', 'generate_corporate_bullshit02', 'generate_corporate_bullshit03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.CorporateBullshitGeneratorSDK(merge([
...
//# sourceMappingURL=GenerateCorporateBullshitEntity.test.js.map