# corporate-bullshit-generator-cli

boru-driven command-line client **and** interactive REPL for the CorporateBullshitGenerator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/corporate-bullshit-generator-cli)
make build

# 2. See usage (words, entities, env vars)
./corporate-bullshit-generator-cli --help

# 3. Provide credentials once, via the environment
export CORPORATE_BULLSHIT_GENERATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./corporate-bullshit-generator-cli load 1 generate_corporate_bullshit            # {id:1} shorthand
./corporate-bullshit-generator-cli load '{id:1}' generate_corporate_bullshit       # explicit match map

# 5. Override the API base URL for a single call
CORPORATE_BULLSHIT_GENERATOR_BASE=https://api.example.com ./corporate-bullshit-generator-cli load 1 generate_corporate_bullshit

# 6. No arguments -> interactive REPL
./corporate-bullshit-generator-cli
corporate-bullshit-generator> load 1 generate_corporate_bullshit
corporate-bullshit-generator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/corporate-bullshit-generator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/corporate-bullshit-generator-cli load 1 generate_corporate_bullshit
   ```
...
```sh
./corporate-bullshit-generator-cli load 1 generate_corporate_bullshit          # scalar shorthand for {id:1}
./corporate-bullshit-generator-cli load '{id:1}' generate_corporate_bullshit     # explicit match map
```
...
./corporate-bullshit-generator-cli load 1 generate_corporate_bullshit
```
...
Run with no arguments to open a REPL (prompt `corporate-bullshit-generator>`). Each line is
evaluated as its own boru expression:

```text
$ ./corporate-bullshit-generator-cli
corporate-bullshit-generator> load 1 generate_corporate_bullshit
corporate-bullshit-generator> /help
corporate-bullshit-generator> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `generate_corporate_bullshit`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/corporate-bullshit-generator-cli`. |
...
(`./corporate-bullshit-generator-cli <expr>`) and interactively (the REPL), and expressions compose the
