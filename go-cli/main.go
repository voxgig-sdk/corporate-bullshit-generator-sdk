// Command CorporateBullshitGenerator-cli is a CLI + REPL for the CorporateBullshitGenerator SDK,
...
const prompt = "corporate-bullshit-generator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "generate_corporate_bullshit"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CORPORATE_BULLSHIT_GENERATOR_APIKEY (api key), CORPORATE_BULLSHIT_GENERATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CORPORATE_BULLSHIT_GENERATOR_APIKEY carries the API key and
	// CORPORATE_BULLSHIT_GENERATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CORPORATE_BULLSHIT_GENERATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CORPORATE_BULLSHIT_GENERATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCorporateBullshitGeneratorSDK(opts)
