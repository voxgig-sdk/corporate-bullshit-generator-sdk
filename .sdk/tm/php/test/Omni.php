...
//    only - a typed CorporateBullshitGeneratorContext there would make every ctx
...
final class CorporateBullshitGeneratorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CorporateBullshitGeneratorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CorporateBullshitGeneratorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CorporateBullshitGeneratorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CorporateBullshitGeneratorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CorporateBullshitGeneratorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CorporateBullshitGeneratorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CorporateBullshitGeneratorError(is_string($code) ? $code : '', $msg);
    }
