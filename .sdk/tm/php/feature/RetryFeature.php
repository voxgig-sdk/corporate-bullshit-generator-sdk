...
// CorporateBullshitGenerator SDK retry feature
...
class CorporateBullshitGeneratorRetryFeature extends CorporateBullshitGeneratorBaseFeature
{
    ...
    public function init(CorporateBullshitGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CorporateBullshitGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CorporateBullshitGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CorporateBullshitGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
