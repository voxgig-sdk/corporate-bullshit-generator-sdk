...
// CorporateBullshitGenerator SDK utility: result_basic

class CorporateBullshitGeneratorResultBasic
{
    public static function call(CorporateBullshitGeneratorContext $ctx): ?CorporateBullshitGeneratorResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof CorporateBullshitGeneratorError) ? $result->err->msg : (string)$result->err;
