...
module CorporateBullshitGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CorporateBullshitGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = CorporateBullshitGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CorporateBullshitGeneratorResponse.new(fetched)
    else
