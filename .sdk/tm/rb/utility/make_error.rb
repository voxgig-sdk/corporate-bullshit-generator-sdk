...
module CorporateBullshitGeneratorUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = CorporateBullshitGeneratorContext.new({}, nil)
    end
    op = ctx.op || CorporateBullshitGeneratorOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || CorporateBullshitGeneratorResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(CorporateBullshitGeneratorError) ? err.msg : err.to_s
    ...
    sdk_err = CorporateBullshitGeneratorError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(CorporateBullshitGeneratorError)
